...
using System.Linq;
/*
public class NEEnvironment : MonoBehaviour
{
    // 1世代あたりの個体数
    [Header("Settings"), SerializeField] private int totalPopulation = 1000;
    private int TotalPopulation { get { return totalPopulation; } }

    // トーナメント選択に用いる個体数
    [SerializeField] private int tournamentSelection = 750;
    ...
    // NNの入力層のサイズ
    [SerializeField] private int inputSize = 12;
    ...
    // 最高キャッチ数
    private int BestCatchCount { get; set; }
    // 最高キャッチ数(現世代)
    ...
    private List<JugglingAgent> Agents { get; } = new List<JugglingAgent>();
    ...
    void Start()
    {
        ...
        {
            var obj = Instantiate(GObject, Vector3.forward * i * 5f, Quaternion.identity);
            obj.SetActive(true);
            var agent = obj.GetComponent<JugglingAgent>();
    ...
    void FixedUpdate()
    {
        ...
        AgentsSet.RemoveAll(p =>
        {
            if (p.agent.IsDone)
            {
                ...
                int c = p.agent.CatchCount;
                BestCatchCount = Mathf.Max(c, BestCatchCount);
    ...
    /// エージェントの状態更新
    /// </summary>
    private void AgentUpdate(JugglingAgent a, NNBrain b)
    {
    ...
    private void UpdateText()
    {
        PopulationText.text = "Population: " + (TotalPopulation - CurrentBrains.Count) + "/" + TotalPopulation
            ...
            + "\nBest catch count: " + BestCatchCount
            + "\nBest catch count (this gen): " + GenBestCatchCount
