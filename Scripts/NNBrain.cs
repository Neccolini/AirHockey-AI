...
public class NNBrain
{
    ...
    private float CrossoverRate{get;set;}=0.50f;
    ...
    public NNBrain Mutate(int generation) {
        ...
        for(int i=0;i<c.HiddenLayers+1;i++){
            c.Biases[i]=MutateLayer(Biases[i], generation); //Weigths -> Biases
    ...
    private Matrix MutateLayer(Matrix m , int generation) {
        ...
        var mutSize=MutRate(generation)*MutationRate+0.02f;
        for(int r=0;r<m.Row;r++){
            for(int c=0;c<m.Column;c++){
    ...
    private float MutRate(int generation) {
        return MutationRate * (float)Math.Max(0, 1 - generation / 100);
    }
    ...
    public Matrix CrossoverLayer(Matrix ma, Matrix mb) {
        var newM=ma.Copy();
        for(int r=0;r<ma.Row;r++){
            for(int c=0;c<ma.Column;c++){
